...
- Applicant Dashboard now brings the latest application status to the top of the page in a simple unframed status strip, with current status, next step, latest update, and quick actions shown without card boxes; the main action can still switch to `Continue Draft` or `Continue Application` when an editable record already exists.
- Applicant Dashboard now also surfaces the same practical fields the applicant sees in `My Applications`, including grant applied for, submitted date, exam result, and next step, so the latest record can be understood immediately from the dashboard.
